...
coast_shapes = [Shape(lon, lat) for (lon,lat) in GeoDatasets.gshhg('c', [1,5])]

# ╔═╡ c9acb76d-ac71-4307-8acc-74fd44f5e24f
function plot_blank_globe()
	plt = plot(legend=false, xlims=(-180,180), ylim=(-90,90), bg_outside=:white, bg=water_color, fg=:black)
...
function ocean2shape(ocn)
	lon = [x.lon for x in ocn.polygon]
	lat = [x.lat for x in ocn.polygon]
	return Shape(lon[[1:end; 1]], lat[[1:end; 1]])
end

# ╔═╡ 06b506ca-e77a-4aec-8d26-997f8682c150
function plot_ocean(ocean_selected, x0, give_up)
	plt = plot_blank_globe()
	if ~isnothing(x0)
		right_ocean = swap(x0) ∈ ocean_selected
		select_color = right_ocean ? right_color : wrong_color
		plot!(plt, bg_outside=select_color)
		if right_ocean || give_up
			plot!(plt, ocean2shape(ocean_selected), fillcolor=select_color, α=0.5, linewidth=1)
		end
		vline!(plt, [x0[1]], color=:black)
		hline!(plt, [x0[2]], color=:black)
	end
	title!(plt, "Where is the $(ocean_selected.name)?")
	return plt
end
...
# ╔═╡ 82263268-b6b1-462d-9fa3-4803d067acba
...
# ╔═╡ e6f437bd-5def-428b-a5af-6b37f1e4eec1
begin
...
# ╔═╡ bd0d7f9b-d47a-4ec8-9885-196e21a6f01e
mainplot = @initially nothing @bind x0 plotclicktracker(plot_ocean(ocean_selected, x0, give_up); draggable=true)

# ╔═╡ 45dea4bc-440d-4eab-b805-5ae792929d93
x0

# ╔═╡ 91760940-c77d-43a0-8955-718222769b2d
...
manifest_format = "2.0"
project_hash = "cfc7d47a26eff2a3f000722c1158f2a3ab8ac588"
